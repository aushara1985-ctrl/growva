...
export async function GET() {
  ...
  const activeExperiments = products.reduce((s, p) => s + p.experiments.filter(e => e.status === 'ACTIVE').length, 0)
  ...
  return NextResponse.json({
    ...
    productList: products.map(p => {
      ...
      return { ...p, conversions7d: stats?.conversions7d || 0, revenue7d: stats?.revenue7d || 0 }
    }),
    recentDecisions,
    dailyData,
    todayBrief,
