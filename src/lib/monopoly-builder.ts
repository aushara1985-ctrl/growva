...
Return ONLY JSON:
{
  ...
  await prisma.monopolyOpportunity.create({
    data: {
      ...
      revenueExpansion: s.revenueExpansionScore || 5,
      competitorGapScore: s.competitorGapScore || 5,
      saudiAdvantage: s.saudiAdvantageScore || 5,
      automationDepth: s.automationDepthScore || 5,
      moatType: s.moatType,
      buildPriority: s.buildPriority || 'backlog',
      whyNow: s.whyNow,
      implementationPlan: s.implementationPlan,
      pricingRecommend: s.pricingRecommendation,
      status: 'new',
    },
  }).catch(() => {})
